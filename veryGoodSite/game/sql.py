...
def updateUserInfo(user):
    ...
    try:
        ...
        c.execute("UPDATE Account SET username=%s, characterName=%s, experience=%s, \
                      ...
                      defense=%s, speed=%s, skillPoints=%s \
                   WHERE userID=%s;", new_data)
    except Exception as e:
        if settings.DEBUG:
...
def generateMonsters(userID, raid_level):
    ...
    try:
        ...
        for i in range(num_monsters):
            ...
            m_health = max(r.randint(m_level // 2,
                                     m_level) // num_monsters, 1)
            m_attack = max(r.randint(m_level // 3,
                                     m_level // 2) // num_monsters, 1)
            m_defense = max(r.randint(m_level // 3,
                                      m_level // 2) // num_monsters, 1)
            m_speed = max(r.randint(m_level // 3,
                                    m_level // 2) // num_monsters, 1)
